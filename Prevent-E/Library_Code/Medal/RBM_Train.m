function RBM_Train(finalDataDirMat, nodes, learning_rate, outDir, preProc, plotNormal, Ks, loadmodel)
...
if loadmodel == 1
    load rbm_model-_1 trained_rbm
    disp("RBM model loaded ");
else
    ...
    % RBM model save
    save rbm_model-_1 trained_rbm

    disp("RBM model trained and saved in: ");
    disp(trainingTime);
end
...
%%%% load(finalDataDirMat + "treshold_definition_dataset.mat");
%%%% [normal_data_size, ~] = size(targetNormal);

Normal_FE = zeros(1, normal_data_size);
for i = 1:normal_data_size
    fe_tmp = trained_rbm.freeEnergy(Normal(i, :));
    %%%% fe_tmp = trained_rbm.freeEnergy(targetNormal(i, :));
    Normal_FE(i) = fe_tmp;
end
...
disp("median/stnd_dev:" + median_FE + " / " + stnd_dev_FE)
...
for k = 1:length(Ks)
    ...
    disp("Tresholds for " + k + " (up/down):" + trendline_up + " / " + trendline_dw)
end
